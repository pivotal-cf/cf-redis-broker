		...
		Context("when the server is running", func() {
			...
			Context("when the server has authentication enabled", func() {
				...
				It("returns an error if the password is incorrect", func() {
					...
					Ω(err).Should(MatchError("WRONGPASS invalid username-password pair or user is disabled."))
				})
...
var _ = Describe("Client", func() {
	...
	BeforeEach(func() {
		certFile, err := filepath.Abs(path.Join("assets", "tls", "redis.crt"))
		Expect(err).ToNot(HaveOccurred())
		keyFile, err := filepath.Abs(path.Join("assets", "tls", "redis.key"))
		Expect(err).ToNot(HaveOccurred())
		caFile, err := filepath.Abs(path.Join("assets", "tls", "ca.crt"))
		Expect(err).ToNot(HaveOccurred())
		pidFile, err := ioutil.TempFile("", "pid")
		Ω(err).ShouldNot(HaveOccurred())
		pidFilePath = pidFile.Name()
		redisArgs = []string{"--bind", "0.0.0.0",
			"--port", "0",
	...
	Describe("connecting to a redis server", func() {
		Context("when the server is not running", func() {
			It("returns an error", func() {
				...
				Ω(err).Should(MatchError(MatchRegexp("dial tcp 0.0.0.0:16480: connect: connection refused")))
			})
		})
		Context("when the server is running", func() {
			JustBeforeEach(func() {
				redisRunner = &integration.RedisRunner{}
				redisRunner.StartTLS(redisArgs)
			})
			...
			It("connects with no error", func() {
				_, err := client.Connect(client.Host(host), client.Port(tlsPort), client.Tls(true))
				Ω(err).ShouldNot(HaveOccurred())
			})
	...
	Describe("using the client", func() {
		...
		Describe("turning on appendonly", func() {
			It("turns on appendonly", func() {
				...
				conn, err := redisclient.Dial("tcp", ":16480", redisclient.DialUseTLS(true), redisclient.DialTLSSkipVerify(true))
				Ω(err).ShouldNot(HaveOccurred())
				defer conn.Close()
	...
	Describe(".Address", func() {
		...
		BeforeEach(func() {
			...
			redisRunner.StartTLS(redisArgs)
			var err error
			redisClient, err = client.Connect(client.Host(host), client.Port(tlsPort), client.Tls(true))
			Expect(err).ToNot(HaveOccurred())
		})
		...
		It("returns an address that consists of the client's host and port", func() {
			Expect(redisClient.Address()).To(Equal(fmt.Sprintf("%s:%d", host, tlsPort)))
		})
	})
	...
	Describe(".GetConfig", func() {
		...
		Context("for a valid key", func() {
			It("returns the correct value", func() {
				actual, err := redisClient.GetConfig("tls-port")
				Ω(err).ShouldNot(HaveOccurred())
				Ω(actual).Should(Equal(fmt.Sprintf("%d", tlsPort)))
	...
	Describe(".RDBPath", func() {
		...
		BeforeEach(func() {
			...
			redisRunner = &integration.RedisRunner{}
			redisRunner.StartTLS(append(redisArgs, "--dir", redisDataDir, "--dbfilename", redisRDBFile))
