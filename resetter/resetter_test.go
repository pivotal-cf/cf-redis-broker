...
var _ = Describe("Client", func() {
	...
	BeforeEach(func() {
		...
		// 	Expect(err).NotTo(HaveOccurred())
		//
		resetter = New("", configPath, fakePortChecker)
		resetter.redis = redisFake
	...
	Describe("#ResetRedis", func() {
		...
		Context("authentication", func() {
			It("authenticates with `AUTH {password}`", func() {
				Expect(connFake.DoCallCount()).To(BeNumerically(">", 0))
				command, args := connFake.DoArgsForCall(0)
				joinedCommand := strings.TrimSpace(fmt.Sprintf("%s %s", command, interfacesToString(args)))
				Expect(joinedCommand).To(Equal("AUTH default"))
			...
			Context("when authentication fails", func() {
				...
				BeforeEach(func() {
					redisFake.DialReturns(nil, authErr)
				})
		...
		// TODO make script kill test pass
		// XIt("executes `SCRIPT KILL`", func() {
		// 	Expect(connFake.DoCallCount()).To(BeNumerically(">", 0))
		// 	command, args := connFake.DoArgsForCall(1)
		// 	joinedCommand := strings.TrimSpace(fmt.Sprintf("%s %s", command, interfacesToString(args)))
		// 	Expect(joinedCommand).To(Equal("SCRIPT KILL"))
		// })

		//
		// It("executes the expected redis commands", func() {
		// 	newConf, err := redisconf.Load(redisClient.liveConfPath)
		// 	Expect(err).NotTo(HaveOccurred())
		//
		// 	expected := []string{
		// 		"AUTH " + conf.Password(),
		// 		"SCRIPT KILL",
		// 		"MULTI",
		// 		"CONFIG SET requirepass " + newConf.Password(),
		// 		// "AUTH {new_password}",
		...
		// 	}
		//
		// 	Expect(connFake.DoCallCount()).To(Equal(len(expected)))
		//
		// 	for i, expectedCommand := range expected {
		// 		command, args := connFake.DoArgsForCall(i)
		// 		joinedCommand := strings.TrimSpace(fmt.Sprintf("%s %s", command, interfacesToString(args)))
		// 		Expect(joinedCommand).To(Equal(expectedCommand))
		// 	}
		// })
		//
		// // It("executes redis commands atomically", func() {
		// // 	doCallCount := connFake.DoCallCount()
		// // 	Expect(doCallCount).To(BeNumerically(">", 1))
		// //
		// // 	firstCommand, _ := connFake.DoArgsForCall(0)
		// // 	Expect(firstCommand).To(Equal("MULTI"))
		// //
		// // 	lastCommand, _ := connFake.DoArgsForCall(doCallCount - 1)
		// // 	Expect(lastCommand).To(Equal("EXEC"))
		// // })
		//
		// It("invokes script kill", func() {
		// 	Expect(connFake.DoCallCount()).To(BeNumerically(">", 1))
		// 	command, args := connFake.DoArgsForCall(1)
		// 	Expect(command).To(Equal("SCRIPT"))
		...
		//
		// 	BeforeEach(func() {
		// 		connFake.DoStub = func(command string, args ...interface{}) (interface{}, error) {
		// 			if command == "SCRIPT" {
		// 				return nil, scriptKillErr
		// 			}
		// 			return nil, nil
		// 		}
		// 	})
